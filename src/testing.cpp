...
/**
 * @brief Fold in subsequent dimensions.
 * @param v_cts The ciphertext to repeatedly fold.
 *
 * @see foldOneFurtherDimension @file spiral.cpp
*/
...
void testHighRate(size_t num_expansions, size_t further_dims, size_t idx_target) {
    ...
    // === key generation ===
    st();
    // MARK: Generate response and query secret keys.
    S_mp = MatPoly(out_n, out_n+1, false);
    Sp_mp = MatPoly(out_n, 1, false);
    sr_mp = MatPoly(1, 1, false);
    keygen(S_mp, Sp_mp, sr_mp, out_n);
    ...
    if (!do_expansion) {
        ...
        // MARK: Encode subsequent dimensions.
        ...
        // MARK: Pack query into single ciphertext.
